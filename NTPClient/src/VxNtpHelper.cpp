...
x_bool_t SetSystemTime_u(x_uint64_t xut_time)
{
	...
	if (xit_temp > 30) {
		...
		BV_OUTPUT("SetTime", xut_time);
		//修改时间
		SetLocalTime(&xtime_system);
#else // !_MSC_VER
		struct tm xtm_system;
		time_t xtm_time = (time_t)(xut_time / 10000000ULL);
		localtime_r(&xtm_time, &xtm_system);

		xtm_system.tm_year = xtm_system.tm_year + 1900;
		xtm_system.tm_mon = xtm_system.tm_mon + 1;
		xtm_system.tm_mday = xtm_system.tm_mday;
		xtm_system.tm_wday = xtm_system.tm_wday;
		xtm_system.tm_hour = xtm_system.tm_hour;
		xtm_system.tm_min = xtm_system.tm_min;
		xtm_system.tm_sec = xtm_system.tm_sec;
		//xtm_context->xut_msec = (x_uint32_t)((xut_time % 10000000ULL) / 10000L);
		//修改时间
		struct timeval tv;
		time_t timep;
		timep = mktime(&xtm_system);
		tv.tv_sec = timep;
		tv.tv_usec = 0;
    ...
    for (std::vector< std::string >::iterator itvec = xvec_host.begin(); itvec != xvec_host.end(); ++itvec)
    {
        ...
        if (0 == xit_err)
        {
            ...
            *xut_timev = xit_tmlst[3] + ((xit_tmlst[1] - xit_tmlst[0]) + (xit_tmlst[2] - xit_tmlst[3])) / 2;
            ...
            BV_OUTPUT("Localhost NTP OUT Time", xit_tmlst[0]);
            BV_OUTPUT("NTP Server DST Time   ", xit_tmlst[1]);
            BV_OUTPUT("NTP Server OUT Time   ", xit_tmlst[2]);
            BV_OUTPUT("Localohst NTP DST Time", xit_tmlst[3]);
            BV_OUTPUT("syn Time              ", *xut_timev);
            BV_OUTPUT("Localhost Time        ", ntp_gettimevalue());
